% l1qc_logbarrier.m  
%  
% Solve quadratically constrained l1 minimization:  解决下面二次规划问题
% min ||x||_1   s.t.  ||Ax - b||_2 <= \epsilon  
%  
% Reformulate as the second-order cone program  改为二阶规划
% min_{x,u}  sum(u)   s.t.    x - u <= 0,  
%                            -x - u <= 0,  
%      1/2(||Ax-b||^2 - \epsilon^2) <= 0  
 % Usage:  xp = l1qc_logbarrier(x0, A, At, b, epsilon, lbtol, mu, cgtol, cgmaxiter)  
%  
% x0 - Nx1 vector, initial point.  y = A*x + e;  如果再乘一个A的转置的话
%                                  X0就等于：x+A'e了  也就是x为初始信号，A'e为噪声，K行1列的矩阵
%  
% A - Either a handle to a function that takes a N vector and returns a K   
%     vector , or a KxN   If A is a function handle, the algorithm  
...
for ii = 1:lbiter  
  ...
  [xp, up, ntiter] = l1qc_newton(x, u, A, At, b, epsilon, tau, newtontol, newtonmaxiter, cgtol, cgmaxiter);  %调用牛顿函数
